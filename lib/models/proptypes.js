var mapKeyToReactPropTypes = function(schema, key) {
    // Attempts to map the field into a React type, a process which may involve looking at other keys
    // in the schema.
    let field = schema[key];
    let type = field.type;
    let reactType = undefined;
    // Add more types as needed.
    if (type === String) reactType = React.PropTypes.string;
    else if (type === Boolean) reactType = React.PropTypes.bool;
    else if (type === Date) reactType = React.PropTypes.object;
    else if (type === Array) {
      let innerType = mapKeyToReactPropTypes(schema, key + ".$");
...
SimpleSchema.prototype.asReactPropTypes = function asReactPropTypes() {
  let schema = this;
  let pt = {};
  for (let i = 0 ; i < schema._schemaKeys.length ; i++) {
    let key = schema._schemaKeys[i];
    ...
    let reactType = mapKeyToReactPropTypes(schema._schema, key);
    pt[key] = reactType;
  }
  return pt;
};
