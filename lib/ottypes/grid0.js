...
//   invertability, but is assumed to be all null if unspecified)
//
// - {dc: idx, c: count, data: [[]]}: Delete `count` columns with
//   contents `data` starting at index `idx`. `data` is needed for
//   invertability, and should be column oriented (i.e. data[0] should
//   be the first column. It's assumed to be all null if unspecified)
  ...
  invert(ops) {
    ...
    _.each(ops, (op) => {
      if (_.has(op, 's')) {
        ...
        // Don't need to specify data here because it's implied to be
        // null if not present
        ...
        // If data is not present, the nulls created by ir are
        // sufficient
        if (_.has(op, 'data')) {
          sets = [];
