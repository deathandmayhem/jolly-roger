...
const METHODS = {
  ...
  listSheets(params) {
    ...
    if (!spreadsheetId) {
      return ContentService
        .createTextOutput(JSON.stringify({ ok: false, error: 'No spreadsheet provided' }))
    ...
    try {
      ...
      return ContentService
        .createTextOutput(JSON.stringify({ ok: false, error: 'Invalid spreadsheet ID' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
  ...
  insertImage(params) {
    ...
    switch (params.source) {
      case 'upload': {
        ...
        if (!filename) {
          return ContentService
            .createTextOutput(JSON.stringify({ ok: false, error: 'No file name' }))
            .setMimeType(ContentService.MimeType.JSON);
        }
        const contents = params['upload-contents'];
        if (!contents) {
          return ContentService
            .createTextOutput(JSON.stringify({ ok: false, error: 'No file contents' }))
            .setMimeType(ContentService.MimeType.JSON);
        }
        ...
        if (!parts) {
          return ContentService
            .createTextOutput(JSON.stringify({ ok: false, error: 'File contents does not match data URL format' }))
            .setMimeType(ContentService.MimeType.JSON);
        }
        ...
        if (!mediaType) {
          return ContentService
            .createTextOutput(JSON.stringify({ ok: false, error: 'No media type' }))
            .setMimeType(ContentService.MimeType.JSON);
        }
      ...
      case 'link': {
        ...
        if (!link) {
          return ContentService
            .createTextOutput(JSON.stringify({ ok: false, error: 'No link provided' }))
            .setMimeType(ContentService.MimeType.JSON);
        }
      ...
      default:
        return ContentService
          .createTextOutput(JSON.stringify({ ok: false, error: 'Invalid source' }))
          .setMimeType(ContentService.MimeType.JSON);
    }
...
function doPost(e) {
  if (e.postData?.type !== 'application/json') {
    return ContentService
      .createTextOutput(JSON.stringify({ ok: false, error: 'POST submission must be JSON' }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (!(method in METHODS)) {
    return ContentService
      .createTextOutput(JSON.stringify({ ok: false, error: 'Invalid method' }))
      .setMimeType(ContentService.MimeType.JSON);
  }
