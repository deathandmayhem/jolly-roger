import ChatMessages, { contentFromMessage } from '../../lib/models/ChatMessages';
...
Migrations.add({
  ...
  async up() {
    ...
    for await (const n of ChatNotifications.find(<any>{ text: { $ne: null } })) {
      const content = contentFromMessage((<any>n).text);
      await ChatNotifications.updateAsync(n._id, {
        $set: { content },
        $unset: { text: 1 },
      }, {
        bypassSchema: true,
