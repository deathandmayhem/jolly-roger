...
import fixAllOfAdditionalProperties from "./fixAllOfAdditionalProperties";
...
>(schema: S, collection: Mongo.Collection<output<S>>) {
  const jsonSchema = zodToMongoSchema(schema, { strict: false });
  fixAllOfAdditionalProperties(jsonSchema);
