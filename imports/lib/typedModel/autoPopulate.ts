...
export const collectSchemaMetadata = (schema: $ZodType): SchemaMetadata => {
  const result: SchemaMetadata = {
    autoPopulated: {},
    defaults: {},
  };

  const inner = (raw: $ZodType, key: string | undefined, depth: number) => {
    ...
    switch (def.type) {
      ...
      case "prefault":
        ...
        if (key !== undefined) {
          // Accessing def.defaultValue invokes the default factory, so we
          // defer the access until each write (rather than capturing a single
          // value here) so that mutable defaults like [] aren't shared
          // between documents.
