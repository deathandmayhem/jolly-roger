...
export function complete(
  ...
  const relative = diff < 0 ? t("from now", " from now") : t("ago", " ago");
  ...
  timeUnits.forEach(({ millis, singular, terse: terseSuffix }) => {
    ...
    if (count > 0) {
      const suffix = terse
        ? terseSuffix
        : `${t("suffixLeadingSpace", " ")}${t(singular, singular, { count: count })}`;
  ...
  if (terms.length === 0) {
    formatted = terse ? t("now", "now") : t("just now", "just now");
  } else if (terse) {
    formatted = terms.join("");
  } else {
    formatted = `${terms.join(t("termSeparator", ", "))}${relative}`;
  }
