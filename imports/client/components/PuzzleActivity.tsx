...
const PuzzleActivity = ({
  ...
  const { t } = useTranslation("PuzzleActivity");
  const { t: tDate, i18n } = useTranslation("DateAndTime");

  const unlockTooltip = (
    <Tooltip id={`${idPrefix}-unlock`}>
      {t("Puzzle unlocked", "Puzzle unlocked")}:{" "}
      {calendarTimeFormat(unlockTime, tDate, i18n.language)}
    </Tooltip>
  );
  ...
  const sparklineTooltip = (
    <Tooltip id={`${idPrefix}-sparkline`}>
      <div>
        {t("People working on this puzzle", "People working on this puzzle")}:
      </div>
