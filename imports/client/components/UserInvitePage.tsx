...
const UserInvitePage = () => {
  const { t } = useTranslation("UserInvitePage");
  ...
  const bulkInvite = useMemo(() => {
    return canBulkInvite ? (
      <div>
        <h2>{t("Bulk invite", "Bulk invite")}</h2>
        ...
        <form onSubmit={onBulkSubmit} className="form-horizontal">
          <FormGroup className="mb-3" controlId={`${idPrefix}-invite-bulk`}>
            <FormLabel>
              {t("bulkInstruction", "Email addresses (one per line)")}
            </FormLabel>
            <FormControl
              as="textarea"
              rows={10}
          ...
          <FormGroup className="mb-3">
            <Button type="submit" variant="primary" disabled={submitting}>
              {t("Send bulk invites", "Send bulk invites")}
            </Button>
          </FormGroup>
        </form>
      </div>
