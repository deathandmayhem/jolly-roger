...
const ChatMessageDiv = styled.div<{
  ...
  ${({ $isOwnMessage, $isHighlighted }) =>
    ...
    css`
      &:hover {
        background-color: ${({ theme }) =>
          theme.colors.hoverChatMessageBackground};
      }
    `}
`;
...
const ChatMessageActions = styled.span<{ $suppressSender?: boolean }>`
  position: absolute;
  top: ${({ $suppressSender }) => ($suppressSender ? "2px" : "20px")};
  right: ${PUZZLE_PAGE_PADDING}px;
  display: none;
  align-items: center;
  gap: 8px;
  padding: 2px 6px;
...
const DeleteIconButton = styled(ActionIconButton)`
  &:hover,
  &:focus {
    color: #d9534f;
  }
...
const StyledInlineFancyEditor = styled(FancyEditor)`
  display: block;
...
function hasNonTrivialContent(content: Descendant[]): boolean {
  return (
    content.length > 0 &&
...
function hasLoadingImage(content: Descendant[]): boolean {
  return (
    content.length > 0 &&
    (content[0]! as MessageElement).children.some((child) => {
...
function slateToMessageContent(content: Descendant[]): ChatMessageContentType {
...
function messageContentToSlate(content: ChatMessageContentType): Descendant[] {
  return [
    {
      type: "message",
...
function useChatImageUpload({
  puzzleId,
  fancyEditorRef,
}: {
  puzzleId: string;
  fancyEditorRef: React.RefObject<FancyEditorHandle | null>;
}) {
  ...
  const uploadImageFile = useCallback(
    ...
    [puzzleId, fancyEditorRef],
  );

  const renderUploadErrorModal = useCallback(() => {
    if (!uploadImageError) {
      return null;
    }
    return createPortal(
...
const InlineChatEditor = ({
  ...
  const { uploadImageFile, renderUploadErrorModal } = useChatImageUpload({
    puzzleId,
    fancyEditorRef,
  });

  useEffect(() => {
    fancyEditorRef.current?.focus(true);
  }, []);

  const canSave = useMemo(() => {
    return hasNonTrivialContent(content) && !hasLoadingImage(content);
  }, [content]);
  ...
  return (
    <InlineEditContainer onKeyDown={handleKeyDown}>
      {renderUploadErrorModal()}
      <StyledInlineFancyEditor
        ref={fancyEditorRef}
...
const ChatHistoryMessage = React.memo(
  ({
    ...
    isEditing?: boolean;
    imageOnLoad: () => void;
    onEdit?: (message: FilteredChatMessageType) => void;
    onDelete?: (message: FilteredChatMessageType) => void;
    onSaveEdit?: (messageId: string, content: Descendant[]) => void;
    onCancelEdit?: () => void;
  }) => {
    ...
    const isOwnMessage =
      message.sender !== undefined && message.sender === selfUserId;
    ...
    const handleSave = useCallback(
      (newContent: Descendant[]) => {
        onSaveEdit?.(message._id, newContent);
      },
      [onSaveEdit, message._id],
    );
    ...
    return (
      ...
      >
        {isOwnMessage && !isEditing && (
          <ChatMessageActions $suppressSender={suppressSender}>
            <ActionIconButton
              type="button"
              title={t("chat.actions.edit", "Edit message")}
              onClick={handleEdit}
            >
            ...
            <DeleteIconButton
              type="button"
              title={t("chat.actions.delete", "Delete message")}
        ...
        {isEditing ? (
          <InlineChatEditor
            ...
            initialContent={messageContentToSlate(message.content)}
            users={users}
            onSave={handleSave}
...
const ChatHistory = React.memo(
  ({
    ...
    const [deleteTargetMessage, setDeleteTargetMessage] = useState<
      FilteredChatMessageType | undefined
    >();
    ...
    const handleDelete = useCallback((msg: FilteredChatMessageType) => {
    ...
    const handleHideDeleteModal = useCallback(() => {
      ...
      setDeleteTargetMessage(undefined);
    }, []);

    const handleConfirmDelete = useCallback(() => {
      if (deleteTargetMessage) {
        deleteChatMessage.call({
          chatMessageId: deleteTargetMessage._id,
        });
      }
      setShowDeleteModal(false);
      setDeleteTargetMessage(undefined);
    ...
    const handleEdit = useCallback((msg: FilteredChatMessageType) => {
    ...
    const handleSaveEdit = useCallback(
      (messageId: string, newContent: Descendant[]) => {
        if (!hasNonTrivialContent(newContent) || hasLoadingImage(newContent)) {
          return;
        }
        const cleanedMessage = slateToMessageContent(newContent);
        editChatMessage.call({
    ...
    return (
      <ChatHistoryDiv ref={historyRef} onScroll={onScrollObserved}>
        <DeleteChatMessageModal
          ...
          messageContent={deleteTargetMessage?.content}
          displayNames={displayNames}
        ...
        {chatMessages.length === 0 ? (
          <ChatMessageDiv
            ...
            $isOwnMessage={false}
          >
            <span>{t("chat.noChatter", "No chatter yet. Say something?")}</span>
          </ChatMessageDiv>
        ) : undefined}
...
const ChatInput = React.memo(
  ({
    ...
    const fancyEditorRef = useRef<FancyEditorHandle | null>(null);
    const { uploadImageFile, setUploadImageError, renderUploadErrorModal } =
      useChatImageUpload({
        puzzleId,
        fancyEditorRef,
      });
    ...
    const isContentNonTrivial = useMemo(() => {
      return hasNonTrivialContent(content);
    }, [content]);

    const isImageLoading = useMemo(() => {
      return hasLoadingImage(content);
    }, [content]);

    const sendContentMessage = useCallback(() => {
      if (isContentNonTrivial && !isImageLoading) {
        // Prepare to send message to server.

        const cleanedMessage = slateToMessageContent(content);
      ...
      return false;
    }, [isContentNonTrivial, isImageLoading, content, puzzleId, onMessageSent]);
      ...
      <ChatInputRow>
        ...
        <InputGroup>
          <StyledFancyEditor
            ...
            placeholder={t("chat.chatPrompt", "Chat")}
            users={users}
          ...
          <Button
            ...
            disabled={disabled || !isContentNonTrivial || isImageLoading}
          >
...
const PuzzlePageMetadata = ({
  ...
  const editButton = canUpdate ? (
    ...
    >
      ...
      {t("puzzle.edit.editButtonText", "Edit")}
    </Button>
  ) : null;
    ...
    guessButton = hasGuessQueue ? (
      ...
      <>
        <Button variant="primary" size="sm" onClick={showAnswerModal}>
          <FontAwesomeIcon icon={faKey} />
          {` ${t("puzzle.answerOrGuess.answerButtonText", "Answer")}`}
        </Button>
        <PuzzleAnswerModal
          ref={answerModalRef}
          puzzle={puzzle}
    ...
    <OverlayTrigger
      ...
      overlay={
        <Tooltip id={`${idPrefix}-hide-puzzle-info`}>
          {t("puzzle.pageView.hideInfo", "Hide puzzle info")}
        </Tooltip>
      }
    >
    ...
    <PuzzleMetadata>
      ...
      <PuzzleMetadataRow>
        <StyledTagList
          ...
          allTags={allTags}
          emptyMessage={t("tags.puzzleNoTagsYet", "No tags yet")}
        />
      </PuzzleMetadataRow>
    </PuzzleMetadata>
  );
...
const PuzzleGuessModal = ({
  ...
  const onSubmitGuess = useCallback(() => {
    ...
    if ((repeatGuess || solvedness !== "unsolved") && !confirmingSubmit) {
      const repeatGuessStr = repeatGuess
        ? t(
            "puzzle.answerOrGuess.repeatAnswer",
            "This answer has already been submitted. ",
          )
        : "";
      const solvednessStr = {
        solved: t(
          "puzzle.answerOrGuess.alreadySolved",
          "This puzzle has already been solved. ",
        ),
        noAnswers: t(
          "puzzle.answerOrGuess.expectNoAnswer",
          "This puzzle does not expect any answers to be submitted. ",
        ),
        unsolved: "",
      }[solvedness];
      const msg = `${solvednessStr} ${repeatGuessStr} ${t("puzzle.answerOrGuess.areYouSure", "Are you sure you want to submit this guess?")}`;
      setConfirmationMessage(msg);
      setConfirmingSubmit(true);
    } else if (!haveSetDirection || !haveSetConfidence) {
      setSubmitError(
        t(
          "puzzle.answerOrGuess.needDirectionAndConfidence",
          "Please set a direction and confidence for your guess.",
        ),
      );
  ...
  const directionTooltip = (
    <Tooltip id={`${idPrefix}-direction-tooltip`}>
      <strong>{t("puzzle.answerOrGuess.direction", "Solve direction")}:</strong>{" "}
  ...
  const title = {
    unsolved: t(
      "puzzle.answerOrGuess.guessTitle.submitGuess",
    ...
    solved: t(
      "puzzle.answerOrGuess.guessTitle.history",
      `Guess history for {{puzzle}}`,
      {
        puzzle: puzzle.title,
      },
    ),
  ...
  return (
    <ModalForm
      ...
      submitLabel={
        confirmingSubmit
          ? t("common.confirmSubmit", "Confirm Submit")
    ...
    >
      <FormGroup as={Row} className="mb-3" controlId={`${idPrefix}-guess`}>
        <FormLabel column xs={3}>
          {t("puzzle.answerOrGuess.guess", "Guess")}
        </FormLabel>
        <Col xs={9}>
          <AnswerFormControl
            type="text"
      ...
      >
        <FormLabel column xs={3}>
          {t("puzzle.answerOrGuess.direction", "Solve direction")}:
        </FormLabel>
        <Col xs={9}>
          ...
          <FormText>
            {t(
              "puzzle.answerOrGuess.directionHelp",
      ...
      >
        <FormLabel column xs={3}>
          {t("puzzle.answerOrGuess.confidence", "Confidence")}
        </FormLabel>
        <Col xs={9}>
          <ValidatedSliderContainer>
          ...
          <FormText>
            {t(
              "puzzle.answerOrGuess.confidenceHelp",
      ...
      {guesses.length === 0 ? (
        <div>
          {t("puzzle.answerOrGuess.noGuesses", "No previous submissions.")}{" "}
        </div>
      ) : (
        [
          <div key="label">
            {t(
              "puzzle.answerOrGuess.previousSubmissions",
...
const PuzzleAnswerModal = ({
  ...
  const onSubmit = useCallback(() => {
    ...
    if ((repeatAnswer || solvedness !== "unsolved") && !confirmingSubmit) {
      ...
      const msg = `${solvednessStr} ${repeatAnswerStr} ${t("puzzle.answerOrGuess.areYouSure", "Are you sure you want to submit this answer?")}`;
      setConfirmationMessage(msg);
      setConfirmingSubmit(true);
      return;
    }
  ...
  return (
    <ModalForm
      ref={formRef}
      title={t(
        "puzzle.answerOrGuess.answerTitle",
        "Submit answer to {{puzzleTitle}}",
    ...
    >
      <FormGroup as={Row} className="mb-3" controlId={`${idPrefix}-answer`}>
        <FormLabel column xs={3}>
          {t("puzzle.answerOrGuess.answer", "Answer")}
        </FormLabel>
        <Col xs={9}>
          <AnswerFormControl
            type="text"
        ...
        {canUpdate && (
          <>
            ...
            <Button variant="primary" onClick={undelete}>
              {t("puzzle.deleted.undelete", "Undelete")}
            </Button>
          </>
        )}
      </Modal.Body>
...
const PuzzlePage = React.memo(() => {
  ...
  const puzzleTitle = activePuzzle
    ? `${activePuzzle.title}${activePuzzle.deleted ? ` (${t("puzzle.errorStatus.deleted", "deleted")})` : ""}`
    : `(${t("puzzle.errorStatus.noSuchPuzzle", "no such puzzle")})`;
  ...
  if (!activePuzzle) {
    return (
      <FixedLayout ref={puzzlePageDivRef}>
        <span>
          {t(
            "puzzle.errorStatus.noSuchPuzzleBody",
            "No puzzle found :( Did you get that URL right?",
          )}
        </span>
      </FixedLayout>
    );
    ...
    <OverlayTrigger
      ...
      overlay={
        <Tooltip id={`${idPrefix}-show-puzzle-info`}>
          {t("puzzle.pageView.showInfo", "Show puzzle info")}
        </Tooltip>
      }
    >
