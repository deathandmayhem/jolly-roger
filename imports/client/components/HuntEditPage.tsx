...
const CONFIGURABLE_PERMISSION_LEVELS: ConfiguredPermissionLevel[] = [
  "hunt_owner",
  "operator",
  "member",
];
...
const PermissionSelector = ({
  ...
  const [dragState, setDragState] = useState<{
    active: boolean;
    leftX: number;
  }>({ active: false, leftX: 0 });

  // Capture events from clicks on option elements and allow dragging between, just like the slider
  const onPointerDown = useCallback(
    (e: React.PointerEvent<HTMLLIElement>) => {
      if (optionsRef.current && !disabled) {
        optionsRef.current.setPointerCapture(e.pointerId);
        e.preventDefault(); // prevent highlighting child text
        ...
        const xoffSteps = xOffset / OPTION_WIDTH_PX;
        const dragPosition = clamp(Math.floor(xoffSteps), 0, maxPosition);
        const dragLevel = CONFIGURABLE_PERMISSION_LEVELS.at(dragPosition);
        if (dragLevel) {
          guardedOnChange(action, dragLevel);
        }
      }
    },
    [guardedOnChange, action, maxPosition, disabled],
  );

  const onPointerMove = useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      if (dragState.active) {
        const xOffset = e.clientX - dragState.leftX;
        const xoffSteps = xOffset / OPTION_WIDTH_PX;
        const dragPosition = clamp(Math.floor(xoffSteps), 0, maxPosition);
        const dragLevel = CONFIGURABLE_PERMISSION_LEVELS.at(dragPosition);
        if (dragLevel) {
    ...
    [dragState.active, dragState.leftX, action, guardedOnChange, maxPosition],
  );

  const onPointerUp = useCallback((_: React.PointerEvent<HTMLElement>) => {
    setDragState({
      active: false,
      leftX: 0,
    });
  ...
  // Safari doesn't handle the jsx-a11y-suggested thing well.
