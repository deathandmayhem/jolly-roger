...
export type TypedMethodSubscribeArgs<T, Arg extends TypedPublicationArgs> =
  Arg extends void ?
    [] :
    [ValidateShape<T, Arg>];

const typedSubscribe = <
  T,
  Args extends TypedPublicationArgs
>(publication: TypedPublication<Args>, ...args: TypedMethodSubscribeArgs<T, Args>) => {
  return Meteor.subscribe(publication.name, ...args);
};

typedSubscribe.async = <
T,
Args extends TypedPublicationArgs
>(publication: TypedPublication<Args>, ...args: TypedMethodSubscribeArgs<T, Args>) => {
